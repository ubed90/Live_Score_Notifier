...
class CricketScores:
    ...
    def display_live_updates(self , choice , matches_urls):
        ...
        status = soup.find('div' , class_ = 'cb-col cb-col-100 cb-min-stts cb-text-complete') or soup.find('div' , class_ = 'cb-text-inprogress') or soup.find('div' , class_ = 'cb-text-stumps') or soup.find('div' , class_ = 'cb-text-lunch') or soup.find('div' , class_ = 'cb-text-tea')
        if status != None:
